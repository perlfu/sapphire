/*
 *  This file is part of the Jikes RVM project (http://jikesrvm.org).
 *
 *  This file is licensed to You under the Eclipse Public License (EPL);
 *  You may not use this file except in compliance with the License. You
 *  may obtain a copy of the License at
 *
 *      http://www.opensource.org/licenses/eclipse-1.0.php
 *
 *  See the COPYRIGHT.txt file distributed with this work for information
 *  regarding copyright ownership.
 */
package org.mmtk.plan.sapphirereftype;

import org.vmmagic.unboxed.*;
import org.vmmagic.pragma.*;

@Uninterruptible
public class SapphireReftypeMutator extends org.mmtk.plan.otfsapphire.OTFSapphireMutator {
  @Inline
  @Override
  public ObjectReference javaLangReferenceReadBarrier(ObjectReference ref, boolean isSoft) {
    if (SapphireReftype.REFERENCE_REPORT_USES) {
    	if (!isSoft)
    		SapphireReftype.reportUseOfWeakRef();
    }
    return super.javaLangReferenceReadBarrier(ref, isSoft);
  }
}
